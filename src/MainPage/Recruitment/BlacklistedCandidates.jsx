...
const BlacklistedCandidates = () => {
  ...
  const fetchBlacklistedCandidates = async (
    ...
    try {
      ...
      const url = `candidate/list?${new URLSearchParams(
        queryParams
      ).toString()}`;
      console.log("Making API call to:", url);

      const response = await apiServices("GET", url, null, {
        access_token: {
          accessToken: token,
        },
        headers: {
  ...
  const handleAddToBlacklist = async (values) => {
    try {
      ...
      if (response?.data?.status) {
        ...
        throw new Error(
          response?.data?.message || "Failed to add candidate to blacklist"
        );
      }
    } catch (error) {
  ...
  return (
    <>
      <Helmet>
        <title>Blacklisted Candidates</title>
      ...
      <div className="content container-fluid">
        ...
        <>
          ...
          <Table
            ...
            locale={{
              emptyText: (
                <Empty description="No blacklisted candidates found" />
              ),
            }}
          />
          {candidates?.length > 0 && (
        ...
        >
          ...
          >
            <Form.Item
              ...
              rules={[
                { required: true, message: "Please select candidate name" },
              ]}
            >
              <Select
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: true,
                  message: "Please enter reason for blacklisting",
                },
              ]}
            >
