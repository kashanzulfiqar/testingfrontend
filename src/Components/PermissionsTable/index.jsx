...
const PermissionsTable = ({ permissions, setPermissions, disabled }) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        const sub = record?.subPermissions?.find(permission => permission.title === "Leads Management");
        const isChecked = record?.subPermissions?.find(permission => permission.title === "Leads Management")?.checked;
        return(            
            <>
              {
