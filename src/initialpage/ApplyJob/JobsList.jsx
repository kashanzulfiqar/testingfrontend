  ...
  const renderJobs = () => {
    ...
    return jobs.map((job) => (
      <div className="col-md-6" key={job._id}>
        <Link className="job-list" to={`/applyjob/jobdetail/${job._id}`}>
          ...
          <div className="job-list-footer">
            <ul>
              <li><i className="fa fa-map-signs" /> {job.location || 'Location not specified'}</li>
              <li><i className="fa fa-money" /> ${job.minSalary}-${job.maxSalary}</li>
              <li><i className="fa fa-clock-o" /> {new Date(job.createdAt).toLocaleDateString()}</li>
            </ul>
          </div>
        </Link>
      </div>
